...
class ChunkSaver(
    ...
    private val chunkLock: Mutex = Mutex()
) : ChunkHandler {

    private var job: Job
    private val savingJobs: MutableSet<Job> = mutableSetOf()
    private val savingJobsLock = Mutex()
    ...
    init {
        ...
        job = WorldScope.launch {
            while (this.isActive) {
                if (timeSinceLastCommit > max_delay || this@ChunkSaver.chunks.size > threshold) {
    ...
    override suspend fun commit() {
        this.savingJobsLock.withLock { this.savingJobs.joinAll() }
        this.chunkLock.withLock {
            val copy = chunks.toMutableSet().filter { return@filter it.blocks.isNotEmpty() }
            for (chunk in copy) {
                for (next in chunk.blocks)
    ...
    override fun add(chunk: Chunk) {
        WorldScope.launch {
            this@ChunkSaver.savingJobsLock.withLock {
                savingJobs.add(
                    WorldScope.launch {
